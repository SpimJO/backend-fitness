...
def _challenge_readable(ch: Challenge, user_id: int) -> bool:
    """Active challenges are public; inactive (soft-deleted) only to creator."""
    if not ch:
        return False
    if ch.is_active:
        return True
    return int(ch.created_by) == int(user_id)


def _participant_progress_status(ch: Challenge, progress_value: int) -> str:
    """
    completed — met target (any time)
    upcoming — challenge not started yet and target not met
    in_progress — running window, target not met
    failed — challenge ended without meeting target
    """
    today = date.today()
    target = int(ch.target_value or 0)
    prog = int(progress_value or 0)
...
def _challenge_payload(
    ...
    if participant:
        payload["your_progress_status"] = _participant_progress_status(
            ch, int(participant.progress_value or 0)
        )
...
def get_challenges():
    ...
    ids = [int(c.id) for c in challenges]
    count_by: Dict[int, int] = {}
    if ids:
        rows = (
            db.session.query(ChallengeParticipant.challenge_id, func.count(ChallengeParticipant.id))
            .filter(ChallengeParticipant.challenge_id.in_(ids))
            .group_by(ChallengeParticipant.challenge_id)
            .all()
        )
        count_by = {int(cid): int(n) for cid, n in rows}

    payload = []
    for ch in challenges:
        p = participant_by_ch.get(int(ch.id))
        cnt = count_by.get(int(ch.id), 0)
        payload.append(
            _challenge_payload(
                ch,
                current_user_id,
...
def get_challenge_detail(challenge_id: int):
    ...
    if not ch or not _challenge_readable(ch, current_user_id):
    ...
    pcnt = ChallengeParticipant.query.filter_by(challenge_id=challenge_id).count()
...
def update_challenge(challenge_id: int):
    ...
    pcnt = ChallengeParticipant.query.filter_by(challenge_id=int(ch.id)).count()
...
def join_challenge(challenge_id: int):
    ...
    if today > ch.end_date:
        return jsonify({"message": "challenge has ended"}), 400
...
def challenge_participants(challenge_id: int):
    ...
    rank = 1
    for p, u in rows:
        prog = int(p.progress_value or 0)
        payload.append(
            {
                "rank": rank,
                "user": {
                ...
                "progress_value": prog,
                "target_value": target,
                "progress_status": _participant_progress_status(ch, prog),
                "is_challenge_creator": int(u.id) == int(ch.created_by),
